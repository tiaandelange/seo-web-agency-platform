...
const entries = [
  ...manifestSource.matchAll(
    /id: '(damtech|proplytic|wedding)'[\s\S]*?repositoryFullName: ([^,\n]+)[\s\S]*?sourceCommit: '([^']+)'/g,
  ),
].map((match) => ({
  id: match[1],
  repositoryFullName: match[2].trim(),
...
function remoteHead(repositoryFullName) {
  ...
  const result = spawnSync(
    'git',
    ['ls-remote', `https://github.com/${repositoryFullName}.git`, 'HEAD'],
    {
      encoding: 'utf8',
    },
  );
...
for (const entry of entries) {
  ...
  if (!repo) {
    console.log(
      `${entry.id}: local source commit ${entry.sourceCommit.slice(0, 7)} — no public GitHub remote recorded`,
    );
    continue;
  }
